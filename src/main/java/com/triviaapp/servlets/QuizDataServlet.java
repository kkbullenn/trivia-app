...
public class QuizDataServlet extends HttpServlet {
    private static final Logger LOGGER = Logger.getLogger(QuizDataServlet.class.getName());
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException
    {
        ...
        try
        {
            ...
            if(currentIndexObj == null)
            {
                ...
                if(!updated)
                {
                    response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
                            "Unable to initialize current question index");
                    return;
                }
            } else
            {
            ...
            if(currentIndex < 0 || currentIndex >= questionIds.size())
            {
                ...
                {
                    response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
                            "Unable to reconcile question index");
                    return;
                }
            ...
            String categoryName = categoryDAO.findCategoryNameById(Integer.parseInt(questionData.get("category_id")));
            ...
            question.put("username", userId != null
                                     ? userDAO.findUsernameById(userId)
                                     : null);
            question.put("lobby_id", lobbyId);
            if(userId != null)
            {
                Map<String, String> existingAnswer =
                        moderatedAnswerDAO.findAnswerForParticipantAndQuestion(lobbyId, questionId, userId);
                ...
                if(answeredCount >= totalQuestions)
                {
                    ...
                    if(leaderboard != null)
                    {
                        ...
                        for(Map<String, String> entry : leaderboard)
                        {
                            ...
                            {
                                entryJson.put(kv.getKey(), kv.getValue() == null
                                                           ? JSONObject.NULL
                                                           : kv.getValue());
                            }
        ...
        {
            LOGGER.log(Level.SEVERE, "Failed to fetch quiz data for lobby " + lobbyId, e);
